...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def _patched_receive_messages(self):
        async for message in self._websocket.iter_bytes():
            # logger.debug(f"Incoming WebSocket message (raw bytes): {message}")
            frame = self._params.serializer.deserialize(message)
            if not frame:
                # logger.warning("Received an unrecognized or invalid frame.")
                continue
            # logger.debug(f"Deserialized frame: {frame}")
            if isinstance(frame, AudioRawFrame):
                # logger.debug("Received AudioRawFrame.")
                await self.push_audio_frame(frame)
            else:
                # logger.debug("Received non-audio frame.")
